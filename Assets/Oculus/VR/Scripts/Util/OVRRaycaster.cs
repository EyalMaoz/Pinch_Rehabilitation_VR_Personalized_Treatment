...
public class OVRRaycaster : GraphicRaycaster, IPointerEnterHandler
{
    ...
    private void Update()
    {
        if (pointer == null)// Eyal added - dont need anymore i think
        {
            try
            {
                pointer = GameObject.FindGameObjectWithTag("LaserPointer");
                Debug.LogWarning("Pointer was null in OVRRaycast");
            }
            catch (Exception)
            {
                Debug.LogWarning("Cant find Pointer in OVRRaycast");
            }
        }
        if (!canvas.worldCamera)
        {
    ...
    //{
    //    StreamWriter sw = File.AppendText(@"/sdcard/PinchProject/System/InformationLog.txt");
    //    sw.WriteLine(msg);
    //    sw.Close();
    //}
}
