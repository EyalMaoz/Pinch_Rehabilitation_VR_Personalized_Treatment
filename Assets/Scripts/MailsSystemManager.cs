...
public class MailsSystemManager
{
    ...
    // We send each request with a background thread so the UI wont get stucked.
    ...
    private static void SendAllReportsInMailForThread(Therapist therapist)
    {
        try
        {
            ...
            mail.Body = "All patients reports are attached as requested.\nIf the files are not displaying correctly, please open it via Google Docs or Notepad.";
